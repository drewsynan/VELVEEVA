    ...
    function read(file, done) {
      ...
      fs.stat(file, function (err, stats) {
        ...
        fs.readFile(file, function (err, buffer) {
          ...
          /* recursive monkeypatch */
          if (options.recursive) {
            var destDirs = subdirs(metalsmith.source());

            destDirs.forEach(function(dir){
              var subName = path.join(dir, name);
              files[subName] = file;
            });
          }
          ...
          if (options.placeInRoot) files[name] = file;

...
function Velveeva(configRecipe) {
  ...
  function screenshots_js() {
    ...
    function renderPage (url, output, vw, vh) {
      var deferred = Q.defer();
      ...
      } else if ((url.match(".jpg") !== null) || url.match(".jpeg") !== null) {
        convertImage([url, output+".png"])
          .then(function() {
            deferred.resolve();
          });
      } else {
    ...
    function takeScreenshot (basePath, dir, matchingFile) {
          ...
          var ss_configs = Object.keys(sizes);

          var full = dir;


          var sizes_array = ss_configs.map(function(k) {
              var d2 = Q.defer();
              var completePath = "file://" + path.join(basePath,dir,matchingFile);
              var outputFile = path.join(basePath,dir,full + sizes[k].name);
              var pngOutputFile = outputFile + ".png";

              var currentWidth = sizes[k].width;
              var currentHeight = sizes[k].height;

              renderPage(completePath, outputFile, currentHeight, currentWidth)
                .then(function(){ // force resize
                  return convertImage([pngOutputFile, '-resize', currentWidth+'x'+currentHeight, pngOutputFile]);
                })
                .then(function () { // convert to jpg
                  return convertImage([pngOutputFile, '-background', 'white', '-flatten', outputFile]);
                })
          ...
          Q.all(sizes_array).done(function(){
            deferred.resolve()
          }, function(){
            deferred.reject();
          });
      ...
      .map(function (dir) {
        ...
        return ___(files)
          .filter(function (file) {
            var regex = new RegExp(dir + "(" + CONFIG.EXTENSIONS.join("|") + ")", "g");
  ...
  function screenshots_py() {
  	var child = require('child_process');
  	var deferred = Q.defer();

    var script_path = path.join(__dirname, './screenshot.py');
    var build_path = path.join(CONFIG.ROOT_DIR, CONFIG.MAIN.DEST);
    var config_path = path.join(CONFIG.ROOT_DIR, "VELVEEVA-config.json");
    console.log(script_path, build_path, config_path);

    p = child.execFileSync(script_path, [build_path, config_path]);
    deferred.resolve()
